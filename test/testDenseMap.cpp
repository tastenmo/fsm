...
TEST_CASE("DenseMap_DenseMap", "[DenseMap]") {
    spie::dense_map<int, int, spie::identity, transparent_equal_to> map;
    const auto &cmap = map;
    ...
    REQUIRE(map.max_size()== (std::vector<spie::details::dense_map_node<int, int>>{}.max_size()));

    map.max_load_factor(.9f);

    REQUIRE(map.max_load_factor()== .9f);

    REQUIRE(map.begin()== map.end());
    REQUIRE(cmap.begin()== cmap.end());
    REQUIRE(map.cbegin()== map.cend());

    REQUIRE_FALSE(map.max_bucket_count() == 0u);
    REQUIRE(map.bucket_count()== 8u);
    REQUIRE(map.bucket_size(3u)== 0u);

    REQUIRE(map.bucket(0)== 0u);
    REQUIRE(map.bucket(3)== 3u);
    REQUIRE(map.bucket(8)== 0u);
    REQUIRE(map.bucket(10)== 2u);
    ...
    REQUIRE_FALSE(map.contains(4.2));
...
TEST_CASE("DenseMap_Constructors"    ) {
    ...
    spie::dense_map<int, int> map;

    ASSERT_EQ(map.bucket_count(), minimum_bucket_count);

    map = spie::dense_map<int, int>{std::allocator<int>{}};
    map = spie::dense_map<int, int>{2u * minimum_bucket_count, std::allocator<float>{}};
    map = spie::dense_map<int, int>{4u * minimum_bucket_count, std::hash<int>(), std::allocator<double>{}};

    map.emplace(3u, 42u);

    spie::dense_map<int, int> temp{map, map.get_allocator()};
    spie::dense_map<int, int> other{std::move(temp), map.get_allocator()};
...
TEST_CASE("DenseMap_Copy", ) {
    spie::dense_map<std::size_t, std::size_t, spie::identity> map;
    map.max_load_factor(map.max_load_factor() - .05f);
    map.emplace(3u, 42u);

    spie::dense_map<std::size_t, std::size_t, spie::identity> other{map};
...
TEST_CASE("DenseMap_Move", ) {
    ...
    spie::dense_map<std::size_t, std::size_t, spie::identity> other{std::move(map)};
...
TEST_CASE("DenseMap_Iterator", ) {
    using iterator = typename spie::dense_map<int, int>::iterator;

    static_assert(std::is_same_v<iterator::value_type, std::pair<const int &, int &>>);
    static_assert(std::is_same_v<iterator::pointer, spie::input_iterator_pointer<std::pair<const int &, int &>>>);
...
TEST_CASE("DenseMap_ConstIterator", ) {
    using iterator = typename spie::dense_map<int, int>::const_iterator;

    static_assert(std::is_same_v<iterator::value_type, std::pair<const int &, const int &>>);
    static_assert(std::is_same_v<iterator::pointer, spie::input_iterator_pointer<std::pair<const int &, const int &>>>);
...
TEST_CASE("DenseMap_IteratorConversion", ) {
    ...
    typename spie::dense_map<int, int>::iterator it = map.begin();
    typename spie::dense_map<int, int>::const_iterator cit = it;
...
TEST_CASE("DenseMap_Insert", ) {
    spie::dense_map<int, int> map;
    typename spie::dense_map<int, int>::iterator it;
    bool result;
...
TEST_CASE("DenseMap_TryEmplaceMovableType", ) {
    spie::dense_map<int, std::unique_ptr<int>> map;
...
TEST_CASE("DenseMap_EraseWithMovableKeyValue", ) {
    static constexpr std::size_t minimum_bucket_count = 8u;
    spie::dense_map<std::string, std::size_t> map;
...
TEST_CASE("DenseMap_LocalIterator", ) {
    using iterator = typename spie::dense_map<std::size_t, std::size_t, spie::identity>::local_iterator;

    static_assert(std::is_same_v<iterator::value_type, std::pair<const std::size_t &, std::size_t &>>);
    static_assert(std::is_same_v<iterator::pointer, spie::input_iterator_pointer<std::pair<const std::size_t &, std::size_t &>>>);
...
TEST_CASE("DenseMap_ConstLocalIterator", ) {
    using iterator = typename spie::dense_map<std::size_t, std::size_t, spie::identity>::const_local_iterator;

    static_assert(std::is_same_v<iterator::value_type, std::pair<const std::size_t &, const std::size_t &>>);
    static_assert(std::is_same_v<iterator::pointer, spie::input_iterator_pointer<std::pair<const std::size_t &, const std::size_t &>>>);
...
TEST_CASE("DenseMap_LocalIteratorConversion", ) {
    ...
    typename spie::dense_map<int, int>::local_iterator it = map.begin(map.bucket(3));
    typename spie::dense_map<int, int>::const_local_iterator cit = it;
...
TEST_CASE("DenseMap_Reserve", ) {
    ...
    ASSERT_EQ(map.bucket_count(), 2 * minimum_bucket_count);
    ASSERT_EQ(map.bucket_count(), spie::next_power_of_two(std::ceil(minimum_bucket_count / map.max_load_factor())));
}
#if false
TEST_CASE("DenseMap_ThrowingAllocator", ) {
    using allocator = test::throwing_allocator<std::pair<const std::size_t, std::size_t>>;
    using packed_allocator = test::throwing_allocator<container::details::dense_map_node<std::size_t, std::size_t>>;
    using packed_exception = typename packed_allocator::exception_type;

    static constexpr std::size_t minimum_bucket_count = 8u;
    container::dense_map<std::size_t, std::size_t, std::hash<std::size_t>, std::equal_to<std::size_t>, allocator> map{};
...
TEST_CASE("DenseMap_NoUsesAllocatorConstruction", ) {
    using allocator = std::pmr::polymorphic_allocator<std::pair<const int, int>>;

    test::tracked_memory_resource memory_resource{};
    container::dense_map<int, int, std::hash<int>, std::equal_to<int>, allocator> map{&memory_resource};
...
TEST_CASE("DenseMap_KeyUsesAllocatorConstruction", ) {
    using string_type = typename test::tracked_memory_resource::string_type;
    using allocator = std::pmr::polymorphic_allocator<std::pair<const string_type, int>>;

    test::tracked_memory_resource memory_resource{};
    container::dense_map<string_type, int, std::hash<string_type>, std::equal_to<string_type>, allocator> map{&memory_resource};
...
TEST_CASE("DenseMap_ValueUsesAllocatorConstruction", ) {
    using string_type = typename test::tracked_memory_resource::string_type;
    using allocator = std::pmr::polymorphic_allocator<std::pair<const int, string_type>>;

    test::tracked_memory_resource memory_resource{};
    container::dense_map<int, string_type, std::hash<int>, std::equal_to<int>, allocator> map{std::pmr::get_default_resource()};

...
class basic_hashed_string : details::basic_hashed_string<Char> {
  using base_type = details::basic_hashed_string<Char>;
  using hs_traits = details::fnv1a_traits<id_type>;
  ...
  [[nodiscard]] static constexpr auto helper(const Char *str) noexcept {
    ...
    for (; str[base.length]; ++base.length) {
      base.hash = (base.hash ^ static_cast<hs_traits::type>(str[base.length])) *
                  hs_traits::prime;
    }
  ...
  [[nodiscard]] static constexpr auto helper(const Char *str,
    ...
    for (size_type pos{}; pos < len; ++pos) {
      base.hash = (base.hash ^ static_cast<hs_traits::type>(str[pos])) *
                  hs_traits::prime;
    }
 ...
 */
template <typename Char>
basic_hashed_string(const Char *str,
                    const std::size_t len) -> basic_hashed_string<Char>;

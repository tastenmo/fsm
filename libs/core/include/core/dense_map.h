...
template <typename Key, typename Type> struct dense_map_node final {
  ...
  dense_map_node(const std::size_t pos, Args &&...args)
      : next{pos}, element{std::forward<Args>(args)...} {}

  template <typename Allocator, typename... Args>
  dense_map_node(std::allocator_arg_t, const Allocator &allocator,
                 const std::size_t pos, Args &&...args)
      : next{pos}, element{spie::make_obj_using_allocator<value_type>(
                       allocator, std::forward<Args>(args)...)} {}

  template <typename Allocator>
  dense_map_node(std::allocator_arg_t, const Allocator &allocator,
                 const dense_map_node &other)
      : next{other.next}, element{spie::make_obj_using_allocator<value_type>(
                              allocator, other.element)} {}

  template <typename Allocator>
  dense_map_node(std::allocator_arg_t, const Allocator &allocator,
                 dense_map_node &&other)
      : next{other.next}, element{spie::make_obj_using_allocator<value_type>(
                              allocator, std::move(other.element))} {}
...
public:
  ...
  using pointer = spie::input_iterator_pointer<value_type>;
  ...
  constexpr dense_map_iterator &
  operator+=(const difference_type value) noexcept {
    it += value;
    return *this;
  }

  constexpr dense_map_iterator
  operator+(const difference_type value) const noexcept {
  ...
  friend constexpr std::ptrdiff_t
  operator-(const dense_map_iterator<ILhs> &,
            const dense_map_iterator<IRhs> &) noexcept;

  template <typename ILhs, typename IRhs>
  friend constexpr bool operator==(const dense_map_iterator<ILhs> &,
                                   const dense_map_iterator<IRhs> &) noexcept;
...
operator-(const dense_map_iterator<ILhs> &lhs,
          const dense_map_iterator<IRhs> &rhs) noexcept {
...
public:
  ...
  template <typename Other,
            typename = std::enable_if_t<!std::is_same_v<It, Other> &&
                                        std::is_constructible_v<It, Other>>>
  constexpr dense_map_local_iterator(
...
template <typename ILhs, typename IRhs>
[[nodiscard]] constexpr bool
operator==(const dense_map_local_iterator<ILhs> &lhs,
           const dense_map_local_iterator<IRhs> &rhs) noexcept {
 ...
 */
template <typename Key, typename Type, typename Hash, typename KeyEqual,
          typename Allocator>
class dense_map {
  static constexpr float default_threshold = 0.875f;
  static constexpr std::size_t minimum_capacity = 8u;

  using node_type = details::dense_map_node<Key, Type>;
  ...
  [[nodiscard]] std::size_t key_to_bucket(const Other &key) const noexcept {
    return spie::fast_mod(sparse.second()(key), bucket_count());
  }
  ...
  [[nodiscard]] auto constrained_find(const Other &key, std::size_t bucket) {
    for (auto it = begin(bucket), last = end(bucket); it != last; ++it) {
      if (packed.second()(it->first, key)) {
        return begin() +
               static_cast<typename iterator::difference_type>(it.index());
      }
    }
  ...
  template <typename Other>
  [[nodiscard]] auto constrained_find(const Other &key,
  ...
  [[nodiscard]] auto insert_or_do_nothing(Other &&key, Args &&...args) {
    ...
    packed.first().emplace_back(
        sparse.first()[index], std::piecewise_construct,
  ...
  [[nodiscard]] auto insert_or_overwrite(Other &&key, Arg &&value) {
    ...
    packed.first().emplace_back(sparse.first()[index], std::forward<Other>(key),
  ...
  void move_and_pop(const std::size_t pos) {
    if (const auto last = size() - 1u; pos != last) {
      size_type *curr = sparse.first().data() +
                        key_to_bucket(packed.first().back().element.first);
...
public:
  ...
  using iterator =
      details::dense_map_iterator<typename packed_container_type::iterator>;
  /*! @brief Constant input iterator type. */
  using const_iterator = details::dense_map_iterator<
      typename packed_container_type::const_iterator>;
  /*! @brief Input iterator type. */
  using local_iterator = details::dense_map_local_iterator<
      typename packed_container_type::iterator>;
  /*! @brief Constant input iterator type. */
  using const_local_iterator = details::dense_map_local_iterator<
      typename packed_container_type::const_iterator>;
   ...
   * @param allocator The allocator to use.
   */
  dense_map(const size_type bucket_count, const hasher &hash,
   ...
   * @param allocator The allocator to use.
   */
  explicit dense_map(const size_type bucket_count,
  ...
  dense_map(dense_map &&) noexcept(
      std::is_nothrow_move_constructible_v<
          spie::compressed_pair<sparse_container_type, hasher>> &&
      std::is_nothrow_move_constructible_v<
          spie::compressed_pair<packed_container_type, key_equal>>) = default;
  ...
  dense_map(dense_map &&other, const allocator_type &allocator)
      : sparse{std::piecewise_construct,
               std::forward_as_tuple(std::move(other.sparse.first()),
                                     allocator),
  ...
  std::pair<iterator, bool> insert(value_type &&value) {
    return insert_or_do_nothing(std::move(value.first),
  ...
  std::enable_if_t<std::is_constructible_v<value_type, Arg &&>,
                   std::pair<iterator, bool>>
  insert(Arg &&value) {
    return insert_or_do_nothing(std::forward<Arg>(value).first,
                                std::forward<Arg>(value).second);
  }
  ...
  std::pair<iterator, bool> emplace([[maybe_unused]] Args &&...args) {
    if constexpr (sizeof...(Args) == 0u) {
      ...
      return insert_or_do_nothing(std::forward<Args>(args).first...,
                                  std::forward<Args>(args).second...);
    } else if constexpr (sizeof...(Args) == 2u) {
      return insert_or_do_nothing(std::forward<Args>(args)...);
    } else {
      auto &node = packed.first().emplace_back(packed.first().size(),
  ...
  size_type erase(const key_type &key) {
    for (size_type *curr = sparse.first().data() + key_to_bucket(key);
  ...
  [[nodiscard]] const mapped_type &at(const key_type &key) const {
    auto it = find(key);
    FSM_ASSERT(it != cend(), "Invalid key");
    return it->second;
  }
   ...
   */
  template <typename Other>
  [[nodiscard]] std::enable_if_t<mpl::is_transparent_v<hasher> &&
                                     mpl::is_transparent_v<key_equal>,
  ...
  [[nodiscard]] std::enable_if_t<
      mpl::is_transparent_v<hasher> && mpl::is_transparent_v<key_equal>,
   ...
   */
  [[nodiscard]] const_local_iterator
  cend([[maybe_unused]] const size_type index) const {
   ...
   */
  [[nodiscard]] const_local_iterator
  end([[maybe_unused]] const size_type index) const {
    return cend(index);
  }
  ...
  void max_load_factor(const float value) {
    FSM_ASSERT(value > 0.f, "Invalid load factor");
    threshold = value;
    rehash(0u);
  }
  ...
  void rehash(const size_type count) {
    ...
    if (const auto sz = spie::next_power_of_two(value); sz != bucket_count()) {
...
private:
  spie::compressed_pair<sparse_container_type, hasher> sparse;
  spie::compressed_pair<packed_container_type, key_equal> packed;
  float threshold;
};
...
struct uses_allocator<spie::details::dense_map_node<Key, Value>, Allocator>
    : std::true_type {};

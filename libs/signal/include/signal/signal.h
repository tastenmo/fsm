 ...
 */
template <typename Type> class slot;
 ...
 */
template <typename Type, typename Allocator> class signal;
...
public:
  ...
  signal(const signal &other, const allocator_type &allocator) noexcept(
      std::is_nothrow_constructible_v<container_type, const container_type &,
  ...
  signal(signal &&other, const allocator_type &allocator) noexcept(
      std::is_nothrow_constructible_v<container_type, container_type &&,
                                      const allocator_type &>)
   ...
   */
  signal &operator=(const signal &other) noexcept(
      std::is_nothrow_copy_assignable_v<container_type>) {
    calls = other.calls;
    return *this;
  }
   ...
   */
  void
  swap(signal &other) noexcept(std::is_nothrow_swappable_v<container_type>) {
    using std::swap;
...
class connection {
  ...
  template <typename> friend class slot;
...
public:
   ...
   */
  [[nodiscard]] bool empty() const noexcept { return _signal->calls.empty(); }
   ...
   */
  template <auto Function> [[nodiscard]] slot before() {
   ...
   * @return A properly initialized slot object.
   */
  template <typename Type> [[nodiscard]] slot before(Type &value_or_instance) {
  ...
  void disconnect() { _signal->calls.clear(); }
 ...
 */
template <typename Ret, typename... Args, typename Allocator>
slot(signal<Ret(Args...), Allocator> &)
    -> slot<signal<Ret(Args...), Allocator>>;

...
public:
  ...
  emitter(emitter &&other, const allocator_type &allocator) noexcept
      : handlers{container_type{std::move(other.handlers.first()), allocator},
                 allocator} {}
  ...
  template <typename Type> void publish(Type &&value) {
    if (const auto id = spie::type_id<Type>().hash();
        handlers.first().contains(id)) {
  ...
  void on(std::function<void(Type &, Derived &)> func) {
    handlers.first().insert_or_assign(
        spie::type_id<Type>().hash(),
  ...
  template <typename Type> [[nodiscard]] bool contains() const {
    return handlers.first().contains(
        spie::type_hash<
            std::remove_cv_t<std::remove_reference_t<Type>>>::value());
  }
...
private:
  spie::compressed_pair<container_type, allocator_type> handlers;
};

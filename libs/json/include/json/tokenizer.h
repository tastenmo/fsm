 ...
 public:
   ...
   std::optional<TokenType> next() {
      ...
      if (auto match = ctre::multiline_starts_with<Regex>(view_.substr())) {
         ...
         mpl::for_sequence(std::make_index_sequence<TokenTypeSize>{},
                           [&](auto i) {
   ...
   [[deprecated("Don't use this function anymore")]] std::string_view
   consume() {
      if (auto match = ctre::multiline_starts_with<Regex>(view_.substr())) {
   ...
   std::optional<std::string_view> consume(TokenType type) {
      ...
      if (auto match = ctre::multiline_starts_with<Regex>(view_.substr())) {
         ...
         mpl::for_sequence(
             std::make_index_sequence<TokenTypeSize>{}, [&](auto i) {
                if (i == type_integer) {
